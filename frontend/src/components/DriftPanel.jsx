// DriftPanel — SFS + IGS Drift Display with Coordination Mismatch Visualization
...
// The key new component — shows Agent 2 vs Agent 3 side by side
// Makes coordination drift immediately visible
// ═══════════════════════════════════════════════════════════════════════════
      ...
      <div style={{ marginBottom: '10px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 24px 1fr', gap: '6px', alignItems: 'center' }}>
          ...
          <div style={{
            ...
            <div style={{ fontSize: '9px', color: '#58a6ff', fontWeight: 700, marginBottom: '3px' }}>
              🧠 AGENT 2 DIAGNOSED
            </div>
            <div style={{
              fontSize: '12px', fontWeight: 700,
              color: result.diagFault === 'NOMINAL' ? '#3fb950' : '#d29922',
            }}>
              {result.diagFault || '—'}
            </div>
          </div>

          {/* Arrow / mismatch indicator */}
          <div style={{
            textAlign: 'center',
            fontSize: '14px',
            color: faultMatch ? '#3fb950' : '#f85149',
          }}>
            {faultMatch ? '→' : '✗'}
          </div>
          ...
          <div style={{
            background: 'var(--bg-surface)',
            border: `1px solid ${faultMatch ? 'var(--border)' : '#f85149'}`,
            borderRadius: '6px', padding: '8px',
          }}>
            <div style={{ fontSize: '9px', color: '#3fb950', fontWeight: 700, marginBottom: '3px' }}>
              🔧 AGENT 3 ACTED ON
            </div>
            <div style={{
              fontSize: '12px', fontWeight: 700,
              ...
              {faultMatch
                ...
                : <span>MISMATCH<br />
                    <span style={{ fontSize: '10px', fontWeight: 400 }}>
                      {faultSignal?.detail || 'No fault-specific actions found'}
                    </span>
                  </span>
              }
            </div>
        ...
        {/* Mismatch explanation for fault */}
        {!faultMatch && (
          <div style={{
            marginTop: '6px',
            ...
            fontSize: '10px', color: '#f85149', lineHeight: 1.5,
          }}>
      ...
      <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 24px 1fr', gap: '6px', alignItems: 'center' }}>
          ...
          <div style={{
            ...
            <div style={{
              fontSize: '12px', fontWeight: 700,
              color: result.diagPriority === 'CRITICAL' ? '#f85149'
                   : result.diagPriority === 'HIGH'     ? '#f0883e'
                   : result.diagPriority === 'MEDIUM'   ? '#d29922'
                   : '#3fb950',
            }}>
          ...
          <div style={{
            textAlign: 'center',
            fontSize: '14px',
            color: priorityMatch ? '#3fb950' : '#f85149',
          }}>
            {priorityMatch ? '→' : '✗'}
          </div>
          ...
          <div style={{
            background: 'var(--bg-surface)',
            border: `1px solid ${priorityMatch ? 'var(--border)' : '#f85149'}`,
            borderRadius: '6px', padding: '8px',
          }}>
            <div style={{ fontSize: '9px', color: '#3fb950', fontWeight: 700, marginBottom: '3px' }}>
              🔧 AGENT 3 REFLECTED
            ...
            <div style={{
              ...
              {priorityMatch
                ...
                : <span>MISMATCH<br />
                    <span style={{ fontSize: '10px', fontWeight: 400 }}>
                      Wrong or missing urgency terms
                    </span>
                  </span>
              }
            </div>
        ...
        {/* Mismatch explanation for priority */}
        ...
        {!result.diagPriority && (
          <div style={{
            ...
            borderRadius: '5px', padding: '7px 9px',
            fontSize: '10px', color: '#d29922', lineHeight: 1.5,
          }}>
            ⚠️ Agent 2 did not state a clear priority level — urgency alignment unverifiable
          </div>
        )}
      </div>

      {/* ── Overall coordination verdict ───────────────────────────────── */}
      <div style={{
        marginTop: '10px',
        paddingTop: '8px',
    ...
    <aside className="drift-panel">
      ...
      <div className="drift-empty">
        ...
        <p className="drift-hint">
          <strong>SFS</strong> — Did Agent 2 demonstrably use the KB?
        </p>
        <p className="drift-hint">
          <strong>IGS</strong> — Did Agent 3 ACT ON Agent 2's specific findings?
          <br />
          <span style={{ fontSize: '10px', color: '#8b949e' }}>
            Not just "based on" — did it address the right fault and right urgency?
          </span>
        </p>
        <p className="drift-hint">
  ...
  return (
    <aside className="drift-panel">
      ...
      <div className="drift-thresholds">
        ...
        <div className="drift-thresh-row" style={{
          borderTop: '1px solid var(--border)',
      ...
      {/* ══════════════════════════════════════════════════════════════════
          COORDINATION MISMATCH PANEL
          Shows Agent 2 findings vs Agent 3 actions side by side
          Makes coordination drift immediately visible to supervisor
      ══════════════════════════════════════════════════════════════════ */}
      ...
      <div className="asi-categories">
        ...
        <div className="agent-drift-block">
          <div className="agent-drift-header">
            ...
            <span className="agent-drift-asi" style={{ color: scoreColor(result.SFS) }}>
              SFS = {result.SFS.toFixed(3)}
            </span>
          </div>
          <ScoreBar value={result.SFS} />
          <div style={{ marginTop: '6px', marginBottom: '10px' }}>
            <span style={{ fontSize: '11px', color: scoreColor(result.SFS), fontWeight: 600 }}>
              {scoreIcon(result.SFS)} {result.agent2.verdict}
            </span>
          </div>
          {result.agent2.signals?.map(s => <SignalCard key={s.id} signal={s} />)}
        </div>
      </div>
