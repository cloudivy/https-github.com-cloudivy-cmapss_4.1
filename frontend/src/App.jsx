...
import { runEvaluators } from './lib/langfuseEvaluators.js'
import langfuse from './lib/langfuseClient.js'
...
Watch the **🔍 KB Query** steps appear live as the Diagnosis Agent retrieves thresholds autonomously.`,
  ...
  const analyseEngine = useCallback(async (engine) => {
    ...
    // ── Create root Langfuse trace for this pipeline run ──────────────────
    const trace = langfuse.trace({
      name:      'cmapss_analysis',
      ...
      tags: [engine.subset, `rul_${Math.floor(engine.rul / 10) * 10}`],
    })
    ...
    // ── Step 2 — Diagnosis Agent (agentic KB query loop) — TRACED ────────
    const diagId = addStreaming('agent', '🧠 Diagnosis Agent (GPT-4o) — Querying KB…')
    let diagnosisText = ''
    let kbCallLog     = []
    try {
      const result  = await streamDiagnosisWithTracing(apiKey, engine, sensorReport, txt => updateMsg(diagId, txt), trace)
    ...
    // ── Step 3 — Maintenance Agent (streaming) — TRACED ──────────────────
    const maintId = addStreaming('agent', '🔧 Maintenance Planner (GPT-4o)')
    let maintenanceText = ''
    try {
      maintenanceText = await streamMaintenanceWithTracing(apiKey, engine, diagnosisText, txt => updateMsg(maintId, txt), trace)
      finishMsg(maintId)
    } catch (e) {
    ...
    // ── Step 4 — Drift Validation — TRACED ───────────────────────────────
    // kbCallLog passed here — this is the KEY CHANGE
    // driftAgent.js uses kbCallLog as audit trail instead of re-diagnosing
    const drift = validateDriftWithTracing(engine, diagnosisText, maintenanceText, kbCallLog, trace)
    ...
    // ── Finalise Langfuse trace and run custom evaluators ─────────────────
    runEvaluators(trace, { drift, kbCallLog })
    trace.end({
      output: {
        sfs_score: drift.SFS,
        igs_score: drift.IGS,
    ...
    // Flush asynchronously — non-blocking, does not affect UI
    langfuse.flushAsync()
